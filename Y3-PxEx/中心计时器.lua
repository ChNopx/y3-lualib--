...
---@class 中心计时器2
---@overload fun(参数:中心计时器2.NEW):self
local m = Class '中心计时器2'

---@type 中心计时器2[]
m.局_全部计时器 = {}

---@class 中心计时器2.NEW
---@field 回调 fun(计时器:中心计时器2, 当前计次:integer)
---@field 次数 integer
---@field 等待时间 number
...
---@param 等待时间 number
---@param 回调 fun(计时器:中心计时器2, 计次:integer)
---@return self
function m.单次执行(等待时间, 回调)
    local self = New '中心计时器2' ({
        次数 = 1,
        等待时间 = 等待时间,
        回调 = 回调
    })
...
y3.计时器.循环执行(0.01, function(timer, count)
    执行计时器()
end)
